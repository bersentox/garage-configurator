import { resolveSceneAsset } from './asset-paths.js';

const HERO_SCENE_FILES = {
...
function hydrateSceneAssets(scene) {
  scene.querySelectorAll('[data-scene-layer]').forEach((node) => {
    const assetName = HERO_SCENE_FILES[node.dataset.sceneLayer];
    if (assetName) {
      node.src = resolveSceneAsset(assetName);
    }
  });
}
